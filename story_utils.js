...
function createStory() {
    ...
    var thisrandom = Math.floor(Math.random() * alliesandrivals.length);
    var thisrandom2 = Math.floor(Math.random() * enemies.length);
    let personalstory = personalStory(enemies[thisrandom2],randFrom(["ally","rival"]),alliesandrivals[thisrandom])
    //console.log(personalstory.prologue)
    //console.log(personalstory.epilogue.success)
    //console.log(personalstory.epilogue.failure)
    ...
    $(story).each(function (i) {
        ...
        if(story[i].stage.name == "Higher Purpose") {
            console.log("hp")
        }
...
function victoryResult(stageindex, story, nextstage, gizmo, masterplan, wincondition, ally, rival) {
    ...
    switch (nextmission.name) {
        ...
        case "Sudden Death": clue = [
              `there's a revival of gladiatorial combats by the ${nextenemy.name}, mirroring the dark traditions you thought you had already shut down.`,
    ...
    switch (masterplan) {
        ...
        case "actsofterror":
            masterplanclues = [
                ...
                `Investigating the site you finally come across a whiteboard covered in complex timelines and event predictions, with clear ${finalboss.name} handiwork. "Plotting out terror, or something else entirely?" you debate, the timelines ${ambiguity_element}`,
...
function rivalText(rival, enemy, stage, rivalboost, gizmo) {
  
  keywords = rival.keywords.concat(stage.keywords).concat(enemy.keywords).concat(["generic"])

//  console.log(keywords)

  const bossorenemy = randFrom([enemy.boss, enemy.name])
  
  // fix swarmed -> swarm
  //console.log(stage)
  const texts = {
    "oni": [
    `You spot ${rival.name} standing there, ${gPron(rival, "possessive")} eyes glowing with an unnatural fire, as though the very essence of the demonic Oni has already claimed ${gPron(rival, "possessive")} soul.`,
    `For a fleeting moment, you think it's just a trick of the light, but then you see ${rival.name}, ${gPron(rival, "possessive")} body radiating a faint, eerie heat, like the infernal flames of the Oni would be slowly seeping into ${gPron(rival, "possessive")} being.`,
    `You catch a glimpse of ${rival.name}, standing in the heart of a forbidden circle, the air around ${gPron(rival, "object")} rippling with heat and power. ${ucInit(gPron(rival, "possessive"))} face is cold, but ${gPron(rival, "possessive")} eyes burn with the promise of hellfire.`,
    `As you observe, the unsettling truth dawns on you: ${rival.name} isn't just seeking the power of the Daemonic Oni. ${ucInit(gPron(rival, "subject"))} has already begun to embody it.`,
    `A chill runs through you as you spot ${rival.name}, standing before the ancient, cursed sigil, ${gPron(rival, "possessive")} movements unnaturally fluid. It’s not the demonic flames that flicker around ${gPron(rival, "possessive")} form that catches your attention — it’s the way ${gPron(rival, "subject")} seems to be embracing it.`,
    `You first see ${rival.name} standing in the shadow, the flames of hellfire licking up ${gPron(rival, "possessive")} arms.`,
    `Then you notice ${rival.name}, standing at the very edge of the area, smoldering as though ${gPron(rival, "subject")} has already been marked by the Oni.`
],
    "gunk": [
  `Carefully, ${rival.name} handles the thick, viscous substance, its properties unclear but undeniably dangerous.`,
  `${rival.name} dips ${gPron(rival, "possessive")} gloves into the strange, glowing gunk, ${gPron(rival, "possessive")} expression unreadable as it sloshes around.`,
  `Cautiously, ${rival.name} maneuvers the bubbling concoction, its exact nature a mystery but its potential for harm evident.`,
  `With deliberate care, ${rival.name} stirs the thick, murky liquid, unsure of what reaction it might provoke but undeterred in their task.`,
  `The ominous, shifting substance is poured into a container by ${rival.name}, the air thick with the smell of something unnatural.`,
  `Testing the viscosity of the strange liquid, ${rival.name}'s fingers move with practiced care as it oozes between them.`,
  `Adjusting the vats of glistening, unknown sludge, ${rival.name}'s actions are methodical as ${gPron(rival, "subject")} keeps their distance from the volatile material.`,
  `Slowly, ${rival.name} pours the dark, tar-like liquid into the carefully marked vessels, watching for any signs of reaction.`,
  `The container of unknown gunk sloshes ominously as ${rival.name} fiddles with it, ${gPron(rival, "subject")} working with unsettling precision.`,
  `With unnerving familiarity, ${rival.name} handles the unstable substance, ${gPron(rival, "possessive")} movements fluid as ${gPron(rival, "subject")} transfers it without a second thought.`
],
    "labyrinth" : [
      `${rival.name} has led ${bossorenemy} through the twisting paths, every turn a challenge in the maze-like ${stage.labyrinth}.`,
      `${rival.name} has navigated ${bossorenemy} through a complex web of ${stage.labyrinth}, guiding them past dangers hidden in every shadow.`,
      `${rival.name} has steered ${bossorenemy} through the confusing ${stage.labyrinth}, ensuring they avoided dead ends at every step.`,
      `${rival.name} has guided ${bossorenemy} through the ${stage.labyrinth}, their path marked only by the faintest traces of a well-worn route.`,
      `${rival.name} has kept ${bossorenemy} on track, ${gPron(rival, "possessive")} steady hand ensuring they navigated the ${stage.labyrinth} without faltering.`,
      `${rival.name} has directed ${bossorenemy} through the ${stage.labyrinth}, avoiding pitfalls and false paths with practiced ease.`,
      `${rival.name} has led ${bossorenemy} through the ${stage.labyrinth}, their movements swift and deliberate, as if they’d done it countless times.`,
      `${rival.name} has expertly guided ${bossorenemy} through ${possessiveSuffix(stage.labyrinth||"")} confusing turns, each step carefully planned to avoid misdirection.`,
      `${rival.name} has charted the course through ${stage.labyrinth}, ensuring ${bossorenemy} followed closely.`,
      `${rival.name} has confidently led ${bossorenemy} through the ${stage.labyrinth}, a clear sense of direction cutting through the disorienting layout.`
    ],
    "retrieval": [
      `${rival.name} stashes ${whichPreposition(gizmo)} handed out by ${enemy.boss}.`,
      `${rival.name} quickly hides ${whichPreposition(gizmo)} given by ${enemy.boss}, ensuring its secrecy.`,
      `${rival.name} carefully pockets ${whichPreposition(gizmo)} handed by ${enemy.boss} without a second thought.`,
      `${rival.name} is seen discreetly storing ${whichPreposition(gizmo)} handed over by ${enemy.boss}, careful not to draw attention.`,
      `${rival.name} is standing next to ${enemy.boss} as the representative of ${gPron(enemy, "possessive")} partner in trade.`,
      `${rival.name} stands by ${enemy.boss}, silently watching as ${enemy.boss} conducts a shady transaction.`,
      `${rival.name} exchanges a few quiet words with ${enemy.boss}, eyeing the  ${whichPreposition(gizmo)} between them.`,
      `${rival.name} appears to be in a tense, silent negotiation with ${enemy.boss}, both eyes locked on the ${whichPreposition(gizmo)}.`
      ],
    "hostages": [
      `${rival.name} is standing guard.`,
      `${rival.name} keeps a sharp eye on the ${stage.hostages}, ensuring no one makes an unexpected move.`,
      `${rival.name} stands sentinel, watching the ${stage.hostages} with a grim expression.`,
      `${rival.name} paces back and forth, never once taking their eyes off the ${stage.hostages}.`,
      `${rival.name} stands still, ${gPron(rival, "possessive")} posture rigid as ${gPron(rival, "subject")} oversees the ${stage.hostages}.`,
      `${rival.name} keeps a vigilant watch, standing at attention by the ${stage.hostages || ""}.`
    ],
    "guns": [
      `${enemy.boss} hands ${rival.name} a gun.`,
      `${enemy.boss} passes a weapon to ${rival.name}, their eyes meeting with a cold understanding.`,
      `${enemy.boss} hands over a sleek gun to ${rival.name}, a silent gesture of trust — or perhaps threat.`,
      `${rival.name} takes the gun from ${enemy.boss}, fingers brushing the cold metal as their gaze hardens.`,
      `${rival.name} accepts the weapon from ${enemy.boss}, locking the gun in place with a practiced motion.`
    ],
    "ritual": [
      `To your dismay, you see ${rival.name} joining ${enemy.boss} in the atrocious ritual!`,
      `You can barely watch as ${rival.name} willingly partakes in the dark ritual with ${enemy.boss}.`,
      `To your horror, ${rival.name} kneels beside ${enemy.boss}, joining them in the forbidden ceremony.`,
      `The sight of ${rival.name} joining ${enemy.boss} in the ritual fills you with dread.`,
      `With a twisted smile, ${rival.name} stands at ${possessiveSuffix(enemy.boss)} side, partaking in the vile ritual without hesitation.`
    ],
    "experiment": 
      [`Apparently, ${rival.name} is taking samples of ${enemy.boss}.`,
      `Apparently, ${enemy.boss} is taking samples of ${rival.name}.`],
    "generic": [
      `To your surprise, you see ${rival.name} in the ranks of ${enemy.name}`,
      `To your dismay, you find out that ${rival.name} has joined forces with ${enemy.boss}.`,
      `Unexpectedly, ${rival.name} is standing among ${possessiveSuffix(enemy.name)} followers, an ally you never anticipated.`,
      `You find yourself taken aback as ${rival.name} is spotted now marching with ${enemy.name}.`,
      `Despite their history, ${rival.name} now stands with ${enemy.boss}.`,
      `You can’t believe your eyes as ${rival.name} joins ${enemy.name}, a dangerous alliance forged in shadows.`
      ],
    "Black Ops": [
      `You spot ${rival.name} just then, standing in the shadows with a presence so calm and controlled that it feels like ${possessiveSuffix(gPron(rival, "subject"))} been watching you long before you noticed ${gPron(rival, "object")}.`,
      `For a brief moment, you wonder if you imagined it, but then you suddenly see ${rival.name} — moving with a quiet intensity, as though ${possessiveSuffix(gPron(rival, "subject"))} already calculated every step of what’s about to happen.`,
      `You catch a glimpse of ${rival.name} in the corner of your eye. There's something about the way ${possessiveSuffix(gPron(rival, "subject"))} standing there, perfectly composed, as though ${gPron(rival, "object")} surroundings means nothing at all.`,
      `As you observe, you suddenly realize ${rival.name} is present. ${ucInit(gPron(rival, "subject"))} hasn't said a word, hasn't moved a muscle.  ${ucInit(gPron(rival, "possessive"))} presence alone fills the room with an aura of quiet, deadly professionalism.`,
      `You can't help but feel an unsettling chill as you notice ${rival.name} standing apart from everyone else — composed, unreadable, like ${gPron(rival, "subject")} is waiting for something to happen... or making it happen.`,
      `You first see ${rival.name} from the corner of your eye, and it’s not ${gPron(rival, "possessive")} appearance that stands out — it’s the way ${gPron(rival, "subject")} seem to absorb everything around ${gPron(rival, "possessive")}, taking in every detail with unnerving calm.`,
      `Then you notice ${rival.name}. At first, it’s hard to pin down why ${gPron(rival, "subject")} stands out so much. But then you realize — it’s the way ${gPron(rival, "subject")} has positioned ${gPron(rival, "reflexive")}, ${gPron(rival, "possessive")} posture, the quiet confidence in ${gPron(rival, "possessive")} eyes. ${ucInit(gPron(rival, "subject"))} is not here by accident.`
    ],
    "Beast": [
      `You see ${rival.name} for the first time, standing at the edge of the room, ${gPron(rival, "possessive")} posture tense, as if ${possessiveSuffix(gPron(rival, "possessive"))} ready to spring at any moment. There’s an undeniable, wild energy that radiates from ${gPron(rival, "object")}, like a predator biding its time.`,
      `The moment your eyes land on ${rival.name}, you feel it — a low, unsettling growl in the back of your mind, not from ${gPron(rival, "reflexive")}, but from the primal part of you that senses danger before it even strikes.`,
      `You suddenly notice ${rival.name}. ${ucInit(gPron(rival, "possessive"))} eyes are wild, burning with an intensity that feels untamed, as if every muscle is coiled and ready to pounce at the slightest provocation.`,
      `You sense a presence that makes you take a step back. You notice ${rival.name} all of a sudden. It’s not ${gPron(rival, "possessive")} size, but the raw, barely-contained violence that seems to hang around ${gPron(rival, "object")}, like a storm ready to break.`,
      `You only then catch sight of ${rival.name}, ${gPron(rival, "possessive")} eyes wide and fierce. There's a fire in them, a hunger that’s hard to ignore, and for a moment, you wonder if ${possessiveSuffix(gPron(rival, "subject"))} holding ${gPron(rival, "reflefixe")} back from ripping through everything in ${gPron(rival, "possessive")} path.`,
      `Your eyes shift to something at the edge of your vision.  ${rival.name} is there, with a primal power in the way ${gPron(rival, "subject")} moves, like an animal pacing in a cage. ${ucInit(gPron(rival, "possessive"))} presence feels too big for the space ${possessiveSuffix(gPron(rival, "subject"))} in, a force of nature barely held in check, ready to break free at any moment.`
    ]
    
  }
  
  const filteredTexts = Object.fromEntries(
  Object.entries(texts).filter(([key, value]) => keywords.includes(key))
  );
  
//  console.log(randFrom(filteredTexts[randFrom(Object.keys(filteredTexts))]))
...
//    rivalText(rival, enemy, stage, rivalboost, gizmo)
...
function switchChoices(key, items) {
